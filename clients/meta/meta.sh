...
#  - `meta` binary is located in the filesystem root
...
# else
#     # Unless otherwise specified by hive, we try to avoid mainnet networkid. If meta detects mainnet network id,
#     # then it tries to bump memory quite a lot
#     FLAGS="$FLAGS --networkid 1331"
# fi
...
# if [ -f /chain.rlp ]; then
#     $meta $FLAGS --gcmode=archive import /chain.rlp
# else
...
# if [ -d /blocks ]; then
#     (cd /blocks && $meta $FLAGS --gcmode=archive --verbosity=$HIVE_LOGLEVEL --nocompaction import `ls | sort -n`)
# else
...
#     echo "secret" > /meta-password-file.txt
#     $meta --nousb account import --password /meta-password-file.txt <(echo "$HIVE_CLIQUE_PRIVATEKEY")

#     # Ensure password file is used when running meta in mining mode.
#     if [ "$HIVE_MINER" != "" ]; then
#         FLAGS="$FLAGS --password /meta-password-file.txt --unlock $HIVE_MINER --allow-insecure-unlock"
#     fi
# fi
